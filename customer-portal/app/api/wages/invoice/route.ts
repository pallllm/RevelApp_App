...
import {
  ...
  InvoiceData,
  InvoiceGameBreakdown,
} from '@/lib/services/invoice-generator';

/**
 * GET /api/wages/invoice?year=2025&month=12
 * 請求書PDFを生成してダウンロード
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // 確定済みでない場合は警告（ただしPDF生成は許可）
    // if (monthlyWage.status !== 'CONFIRMED' && monthlyWage.status !== 'PAID') {
    //   return errorResponse(
    //     new Error('Wage is not confirmed yet'),
    //     400
    //   );
    // }
    ...
    // メンバーごとのゲーム内訳を集計
    const memberGameBreakdowns = new Map<string, InvoiceGameBreakdown[]>();
    for (const mw of monthlyWage.memberMonthlyWages) {
      const userPlays = gamePlayRecords.filter((r) => r.userId === mw.user.id);
      const gameMap = new Map<string, InvoiceGameBreakdown>();
      ...
      memberGameBreakdowns.set(mw.user.id, Array.from(gameMap.values()));
    }

    // 請求書データを構築
    const invoiceData: InvoiceData = {
      invoiceNumber: generateInvoiceNumber(facility.id, year, month),
      ...
      paymentDueDate: monthlyWage.paymentDate
    ...
    // PDFレスポンスを返す
    const filename = `invoice_${year}${String(month).padStart(2, '0')}_${facility.name}.pdf`;
